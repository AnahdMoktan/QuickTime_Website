   ...
   <div class="content-wrapper">
    ...
    <div class="content-header">
        <div class="container-fluid">
            ...
            @php
            $equipments = App\Equipments::get();
            @endphp
            <hr style="padding-top: 40px;">
            <h3 style="text-align:center">Equipments Contents</h3>
            <table class="table">
                <thead>
                    <tr>
                        <th scope="col">Id</th>
                ...
                <tbody>
                    @foreach($equipments as $i=>$eq)
                        <tr>
                            ...
                            <td>
                                {!! substr($eq['description'],0,280) !!}
                          </td>

                            <td>
                                <img height="80" width="80"
                                    src="{{ url(implode('storage',explode('public',$eq['image']))) }}">
                            </td>
                            <td>
                            <a href="#editequipments{{$eq->id}}" data-toggle="modal">
                                    <i class="fas fa-edit">Edit</i>
                                </a>
                                &nbsp;
                                <a
                                    href="{{ url('equipments/delete',array($eq->id)) }}">
                            ...
                            <div class="modal" tabindex="-1" id="editequipments{{$eq->id}}">
                                        <div class="modal-dialog">
                                            <div class="modal-content">
                                                ...
                                                <form method="POST"  action="{{ url('equipments/update',array($eq->id)) }}" enctype="multipart/form-data" id="uploadForm">
                                                    {{ csrf_field() }}
                                                    <div class="form-group">
                                                      <label for="">Title</label>
                                                    <input class="form-control" name="title" value="{{$eq['title']}}"/>
                                                  </div>
                                                  <div class="form-group">
                                                    <label for="">Description</label>
                                                  <textarea name="description" class="form-control ckeditor">{{ $eq['description'] }}</textarea>
                                                </div>
                                                  <div class="form-group">
                                                      <label for="">Image</label>
